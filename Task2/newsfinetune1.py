"""
LLM Fine-tuning for Knowledge Graph Triple Extraction - Case 1

This script fine-tunes a Mistral-7B language model for extracting knowledge graph triples
from news articles. The model is trained exclusively on gold standard triples using a 
custom Alpaca-style instruction prompt format. This represents the first case in our 
experimental setup where the LLM learns to identify and extract high-quality, manually 
annotated subject-predicate-object relationships from textual content.

Key Features:
- Uses gold standard triple annotations for supervised fine-tuning
- Implements custom prompt engineering with instruction-input-response format
...
Training Data: Gold prompt train/test sets with manually curated triples
Model: Mistral-7B-Instruct with 4-bit quantization and LoRA adapters
Evaluation: Precision, Recall, and F1 scores on triple extraction task
"""
...
# ========= DATA LOADING =========
try:
    data_train = pd.read_csv('/cfs/earth/scratch/adlurmoh/track2/case1/gold_prompt_train.csv')
    data_test = pd.read_csv('/cfs/earth/scratch/adlurmoh/track2/case1/gold_prompt_test.csv')
...
# ========= DATASET PREPARATION =========
# Convert dataset to HF Dataset format
dataset_train = Dataset.from_pandas(data_train)
dataset_test = Dataset.from_pandas(data_test)

# Define instruction prompt template
...
model_save_path = "newsKG21_finetuned_model1"
model.save_pretrained(model_save_path)
tokenizer.save_pretrained(model_save_path)
log_print(f"Model and tokenizer saved to '{model_save_path}'")
...
def generate_prediction(example):
    """Generate prediction for a test example using the fine-tuned model"""
...
output_file = "newsfinetune1_predictions.jsonl"
...
log_print(f"Predictions saved to {output_file}")
